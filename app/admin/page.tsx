      ...
      <section className="admin-panel">
        ...
        {stories.length === 0 ? <div className="admin-empty"><FilePenLine size={30} /><p>No stories yet.</p><Link href="/admin/stories/new">Create your first story</Link></div> : (
          <div className="story-table-wrap"><table className="story-table"><thead><tr><th>Story</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
            {stories.map((story) => <tr key={story.id}><td><strong>{story.title_en}</strong><small>{story.category} · {story.author}</small></td><td><span className={`status ${story.status}`}>{story.status}</span>{story.is_breaking && <span className="breaking-badge"><Radio size={11} /> Breaking</span>}</td><td>{new Date(story.updated_at).toLocaleDateString()}</td><td><div className="table-actions"><Link title="Edit" href={`/admin/stories/${story.id}`}><FilePenLine size={17} /></Link>{story.status === "published" && <Link title="View" target="_blank" href={`/news/${story.slug}`}><Eye size={17} /></Link>}<form action={togglePublishAction}><input type="hidden" name="id" value={story.id} /><input type="hidden" name="next_status" value={story.status === "published" ? "draft" : "published"} /><button>{story.status === "published" ? "Unpublish" : "Publish"}</button></form>{story.status === "draft" && <form action={deleteDraftAction}><input type="hidden" name="id" value={story.id} /><ConfirmSubmit className="danger-icon" message="Permanently delete this draft?"><Trash2 size={17} /></ConfirmSubmit></form>}</div></td></tr>)}
          </tbody></table></div>
        )}
      </section>
    </AdminShell>
