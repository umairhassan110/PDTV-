  ...
  return (
    <AdminShell profile={profile}>
      ...
      <section className="admin-panel">
        ...
        {recentRun && <div className="ai-run-strip"><Bot size={16} /><span>Last run: <strong>{recentRun.status}</strong></span><span>Discovered {recentRun.discovered || 0}</span><span>Processed {recentRun.selected || 0}</span><span>Ready {recentRun.ready || 0}</span><small>{new Date(recentRun.started_at).toLocaleString()}</small></div>}
          ...
          <div className="ai-draft-list">{drafts.map((draft) => (
            <article className="ai-draft-card" key={draft.id}>
              <div className="ai-draft-main">
                <div className="ai-draft-topline"><span className={`ai-state ${draft.status}`}>{draft.status === "ready" ? <CheckCircle2 size={14} /> : draft.status === "needs_review" ? <CircleAlert size={14} /> : draft.status === "approved" ? <ShieldCheck size={14} /> : <XCircle size={14} />}{draft.status.replace("_", " ")}</span><span>{draft.category}</span><span>{new Date(draft.created_at).toLocaleString()}</span></div>
